...
OLD_REV='c2f52a6d767f631b30e0f79d148d03b34f00ef0a'
REVISION='3e617865c20eb533cb4e1e8fee6c3c099e970bec'
REVISIONS = [REVISION]
HEURISTICS = [
    ("lmcut", "lmcut()"),
]
...
SIMPLE_MUTEX_CONFIGS = list(chain.from_iterable(
    ...
    IssueConfig(
        ...
        ["--search", "astar({heuristic},pruning=limited_pruning(stubborn_sets_simple(stubborn_set_type={ss_type},use_mutex_interference=true),min_required_pruning_ratio={min_pruning}),verbosity=silent)".format(**locals())], driver_options=['--search-time-limit', '5m']),
    )
    for nick, heuristic in HEURISTICS
...
ATOM_CONFIGS = list(chain.from_iterable(
    ...
    IssueConfig(
        ...
        ["--search", "astar({heuristic},pruning=limited_pruning(atom_centric_stubborn_sets(stubborn_set_type={ss_type},atom_selection_strategy={atom_selection_strategy},use_sibling_shortcut=true),min_required_pruning_ratio={min_pruning}),verbosity=silent)".format(**locals())], driver_options=['--search-time-limit', '5m']),
    )
    for nick, heuristic in HEURISTICS
    for ss_type in ["strong", "weak", "compliant"]
...
exp.add_fetcher(
    'data/2022-08-03-paper-blind-eval',
